...
VECTOR4D operator* (const VECTOR4D& V, const MATRIX4D& M) {
	VECTOR4D R (0,0,0,0);
	R.x = V.x * M.m[0][0] + V.x * M.m[1][0] + V.x * M.m[2][0] + V.x * M.m[3][0];
	R.y = V.y * M.m[0][1] + V.y * M.m[1][1] + V.y * M.m[2][1] + V.y * M.m[3][1];
	R.z = V.z * M.m[0][2] + V.z * M.m[1][2] + V.z * M.m[2][2] + V.z * M.m[3][2];
	R.w = V.w * M.m[0][3] + V.w * M.m[1][3] + V.w * M.m[2][3] + V.w * M.m[3][3];
	return R;
}
